...
import { Save, ArrowLeft, Building2, Upload, X } from "lucide-react";
...
export default function FacilityForm() {
  ...
  const [form, setForm] = useState({ title: "", description: "", icon: "building", is_active: 1, sort_order: 0 });
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState("");

  useEffect(() => {
    if (isEdit) api.get(`/facilities/${id}`).then(v => {
      setForm({ title: v.title || "", description: v.description || "", icon: v.icon || "building", is_active: v.is_active ?? 1, sort_order: v.sort_order || 0 });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) { const res = await uploadFile("/upload", imageFile, "taller-motos/facilities"); imageUrl = res.data?.url || res.url || res.image || ""; }
      const payload = { ...form, image: imageUrl || null };
      if (isEdit) await api.put(`/facilities/${id}`, payload);
      else await api.post("/facilities", payload);
      showToast("success", isEdit ? "Instalación actualizada" : "Instalación creada");
      navigate("/facilities");
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title={isEdit ? "Editar Instalación" : "Nueva Instalación"} description={isEdit ? "Modifica la instalación." : "Agrega una nueva instalación."}
        breadcrumbs={[{ label: "Instalaciones", to: "/facilities" }, { label: isEdit ? "Editar" : "Nuevo" }]} icon={<Building2 size={20} />}
        action={<button onClick={() => navigate("/facilities")} className="mp-btn-ghost text-xs"><ArrowLeft size={14} /> Volver</button>} />
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="mp-card p-6 space-y-4">
            ...
            <div>
              ...
              <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className="mp-input text-sm" placeholder="Ej: Sala de Diagnóstico" required />
            </div>
            <div>
              <label className="block text-xs font-medium text-[var(--mp-text-secondary)] mb-1.5">Descripción</label>
              <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="mp-input text-sm min-h-[100px]" placeholder="Describe esta instalación..." />
            </div>
          </div>
        </div>
        <div className="space-y-6">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => navigate("/facilities")} className="mp-btn-ghost flex-1 text-xs">Cancelar</button>
