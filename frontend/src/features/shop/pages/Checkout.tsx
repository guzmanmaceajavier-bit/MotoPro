...
export default function Checkout() {
  ...
  useEffect(() => {
    if (lastOrderId && paymentMethod === "mercadopago" && step === 3) {
      pollRef.current = setInterval(async () => {
        try {
          const data = await api.get(`/checkout/${lastOrderId}`);
          ...
          if (status === "paid" || status === "completed" || status === "confirmed") {
            ...
            clearInterval(pollRef.current);
            addToast("El pago no se completó", "error");
          }
        } catch {}
      }, 3000);
      return () => clearInterval(pollRef.current);
    }
  }, [lastOrderId, step, paymentMethod]);
              ...
              <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }}>
                    ...
                    {items.map(item => (
                      <div key={item.id} className="flex items-center gap-4 bg-surface-secondary border border-border rounded-lg p-4">
                        <div className="w-20 h-20 rounded-lg bg-surface-tertiary flex items-center justify-center shrink-0 overflow-hidden">
                          {item.image ? <img src={item.image} alt={item.name} loading="lazy" className="w-full h-full object-cover" /> : <span className="text-3xl opacity-40">🛵</span>}
                        </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {paymentMethod === "transferencia" && (
                      <div className="bg-surface-secondary border border-border rounded-lg p-4 mt-4">
                        ...
                        <p className="text-xs text-text-tertiary mt-2">Envía el comprobante a {config.site_email || "info@motopro.com"}</p>
                      </div>
                    )}
                ...
                {step === 3 && (
                  <div className="text-center py-12">
                    {paymentMethod === "mercadopago" && !paymentStatus ? (
                      <>
                        <div className="mx-auto mb-6 w-16 h-16 rounded-full bg-surface-tertiary flex items-center justify-center">
                          <Spinner size="md" />
                        </div>
                        <h2 className="text-2xl font-bold text-text-primary mb-2">Procesando pago</h2>
                        <p className="text-text-secondary mb-2">Te redirigimos a MercadoPago para completar el pago.</p>
                        <p className="text-xs text-text-tertiary mb-6">Esperando confirmación... Si ya pagaste, no cierres esta página.</p>
                        {lastOrderId && (
                          <Link to={`/mi-cuenta?tab=compras`} className="text-sm text-interactive-accent hover:underline">
                      ...
                      <>
                        ...
                        <h2 className="text-2xl font-bold text-text-primary mb-2">¡Pago confirmado!</h2>
                        <p className="text-text-secondary mb-6 max-w-md mx-auto">Gracias por tu compra. Recibirás un resumen por correo.</p>
                        <div className="flex items-center justify-center gap-4">
                          <Link to="/" className="inline-flex items-center gap-2 rounded-lg bg-interactive-accent px-6 py-3 font-semibold text-white hover:bg-interactive-accent-hover transition-all">
                            Volver al inicio
                          </Link>
                          <Link to="/mi-cuenta?tab=compras" className="inline-flex items-center gap-2 rounded-lg border border-interactive-accent px-6 py-3 font-semibold text-interactive-accent hover:bg-interactive-accent/10 transition-all">
                            Ver mis pedidos
                          </Link>
                        </div>
                      </>
                      ...
                      <>
                        ...
                        <h2 className="text-2xl font-bold text-text-primary mb-2">Pago no completado</h2>
                        <p className="text-text-secondary mb-6 max-w-md mx-auto">El pago no pudo procesarse. Puedes intentar de nuevo.</p>
                        <Link to="/checkout" className="inline-flex items-center gap-2 rounded-lg bg-interactive-accent px-6 py-3 font-semibold text-white hover:bg-interactive-accent-hover transition-all">
                          Intentar de nuevo
                        </Link>
                      </>
                    ) : (
