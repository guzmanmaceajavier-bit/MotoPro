  ...
  return (
    <>
      ...
      <main className="pt-16">
        ...
        <section className="py-4 bg-surface-primary relative z-20 border-b border-border-subtle">
          <div className="mx-auto max-w-7xl px-4">
            <div className="flex items-center gap-3 rounded-xl bg-surface-secondary border border-border-subtle px-4 py-1 hover:border-interactive-accent/30 transition-all duration-300">
              ...
              />
              {searchQuery && (
                <button onClick={() => setSearchQuery("")} className="text-text-tertiary hover:text-text-primary transition-colors shrink-0">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              {["Todos", ...categories.map((c) => c.name)].map((cat) => (
                <button key={cat} onClick={() => setActiveCat(cat)}
                  className={`shrink-0 rounded-lg px-4 py-1.5 text-xs font-medium transition-all duration-base ${
                    activeCat === cat
