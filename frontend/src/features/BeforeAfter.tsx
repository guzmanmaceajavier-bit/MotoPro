...
export function BeforeAfter() {
  const [items, setItems] = useState<BeforeAfterItem[]>(fallbackData);
  const [current, setCurrent] = useState(0);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const controller = new AbortController();
    fetch("/api/before-after", { signal: controller.signal })
      .then((r) => r.json())
      .then((res) => {
        const data = res.data || res || [];
        if (Array.isArray(data) && data.length > 0) {
          setItems(data.map((d: BeforeAfterItem) => ({
            id: d.id, title: d.title,
            before_image: d.before_image, after_image: d.after_image,
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <AnimatePresence mode="wait">
              ...
              >
                <img src={comp.before_image} alt="Antes" className="w-full h-full object-cover" />
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <img src={comp.after_image} alt="Después" className="w-full h-full object-cover" />
