...
import { useConfig, useBrands, useServices } from "@/providers/CMSProvider";
...
export function AboutSection() {
  ...
  useEffect(() => {
    api.get("/products?all=1").then((prods: any) => {
      const p = Array.isArray(prods?.data ?? prods) ? (prods?.data ?? prods) : [];
      setStats({ products: p.length, brands: brands.length, services: services.length });
    }).catch(() => setStats({ products: 0, brands: brands.length, services: services.length }));
    const interval = setInterval(() => setImgSet((p) => (p + 1) % imageSets.length), 4000);
    return () => clearInterval(interval);
  }, [brands.length, services.length]);
