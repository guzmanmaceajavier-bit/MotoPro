...
export function TeamSection() {
  const [team, setTeam] = useState<{ id?: string; name: string; role: string; specialty: string; experience: string; description: string; image?: string }[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get("/team").then((data) => setTeam((data as any[]) || [])).catch(() => setTeam([])).finally(() => setLoading(false));
  }, []);
