                  ...
                  {p ? (
                    <div className="text-center w-full">
                      <div className="w-20 h-20 mx-auto rounded-lg bg-surface-tertiary flex items-center justify-center mb-3 overflow-hidden">
                        {p.image ? <img src={p.image} alt={p.name} loading="lazy" className="w-full h-full object-cover" /> : <span className="text-3xl opacity-30">🏍️</span>}
                      </div>
              ...
              {filtered.filter(p => !selected.find(s => s.id === p.id)).slice(0, 20).map(p => (
                ...
                >
                  <div className="w-12 h-12 rounded-lg bg-surface-tertiary flex items-center justify-center shrink-0 overflow-hidden">
                    {p.image ? <img src={p.image} alt={p.name} loading="lazy" className="w-full h-full object-cover" /> : <span className="text-xl opacity-30">🏍️</span>}
                  </div>
                    ...
                    {selected.map(p => (
                      <td key={p.id} className="p-4 text-center">
                        <div className="w-16 h-16 mx-auto rounded-lg bg-surface-tertiary flex items-center justify-center overflow-hidden">
                          {p.image ? <img src={p.image} alt={p.name} loading="lazy" className="w-full h-full object-cover" /> : <span className="text-2xl opacity-30">🏍️</span>}
                        </div>
                      </td>
                    ))}
                  </tr>
