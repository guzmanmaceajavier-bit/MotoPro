...
export default function AgendarCita() {
  ...
  useEffect(() => {
    api.get("/services").then(d => setServices(Array.isArray(d) ? d : [])).catch(() => {});
    api.get("/team").then(d => setMechanics(Array.isArray(d) ? d : [])).catch(() => {});
  }, []);
                      ...
                      <div>
                        ...
                        {loading ? (
                          <div className="flex items-center gap-2 text-sm text-text-secondary">
                            <div className="w-4 h-4 border-2 border-interactive-accent border-t-transparent rounded-full animate-spin" />
                            Cargando horarios...
                          </div>
                        ) : slots.length > 0 ? (
