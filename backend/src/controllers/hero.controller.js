...
exports.getById = (req, res) => {
  const slide = get("SELECT * FROM hero_slides WHERE id = ?", [req.params.id]);
...
exports.create = (req, res) => {
  ...
  const id = generateId();
  run("INSERT INTO hero_slides (id, title, subtitle, description, image, gradient, cta_text, cta_link) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    [id, title, subtitle || "", description || "", image || null, gradient || "from-purple-600 to-blue-600", cta_text || "Ver más", cta_link || "/tienda"]);
...
exports.update = (req, res) => {
  ...
  const existing = get("SELECT id, image FROM hero_slides WHERE id = ?", [req.params.id]);
  if (!existing) return error(res, "Slide no encontrado", 404);
  if (image && image !== existing.image) destroyImage(existing.image);
  run(`UPDATE hero_slides SET title = COALESCE(?, title), subtitle = COALESCE(?, subtitle),
    description = COALESCE(?, description), image = COALESCE(?, image),
    gradient = COALESCE(?, gradient), cta_text = COALESCE(?, cta_text),
    cta_link = COALESCE(?, cta_link), sort_order = COALESCE(?, sort_order),
    is_active = COALESCE(?, is_active), updated_at = datetime('now') WHERE id = ?`,
    [title || null, subtitle || null, description || null, image || null,
      gradient || null, cta_text || null, cta_link || null,
...
exports.remove = (req, res) => {
  ...
  run("DELETE FROM hero_slides WHERE id = ?", [req.params.id]);
