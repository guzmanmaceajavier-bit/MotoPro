...
export function StatsSection() {
  ...
  useEffect(() => {
    Promise.all([
      api.get("/products").then((r) => String(Array.isArray(r) ? r.length : r?.data?.length || 0)).catch(() => "0"),
      api.get("/brands").then((r) => String(Array.isArray(r) ? r.length : 0)).catch(() => "0"),
      api.get("/testimonials?all=1").then((r) => String(Array.isArray(r) ? r.length : 0)).catch(() => "0"),
    ]).then(([products, brands, testimonials]) => setCounts({ products, brands, testimonials }));
  }, []);
