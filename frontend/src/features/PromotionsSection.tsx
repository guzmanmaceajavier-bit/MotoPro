...
export function PromotionsSection() {
  const [items, setItems] = useState<OfferItem[]>(fallbackOffers);
  ...
  useEffect(() => {
    fetch("/api/offers")
      ...
      .then((res) => {
        ...
        const filtered = Array.isArray(data) ? data.filter((o: OfferItem) => o.is_active).sort((a: OfferItem, b: OfferItem) => (a.sort_order || 0) - (b.sort_order || 0)).slice(0, 3) : [];
              ...
              >
                ...
                >
                  <div className="relative z-10">
                    {p.image && <img src={p.image} alt={p.title} className="w-16 h-16 object-cover rounded-lg mb-3" />}
