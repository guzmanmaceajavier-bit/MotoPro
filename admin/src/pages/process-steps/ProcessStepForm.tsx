...
const iconOptions = ["settings", "tool", "search", "clipboard", "checkCircle", "star", "zap", "clock", "phone", "messageCircle", "mapPin", "shield"];
...
export default function ProcessStepForm() {
  ...
  const [form, setForm] = useState({ title: "", description: "", icon: "settings", color: "#FF6B00", is_active: 1, sort_order: 0 });

  useEffect(() => {
    if (isEdit) api.get(`/process-steps/${id}`).then(v => {
      setForm({ title: v.title || "", description: v.description || "", icon: v.icon || "settings", color: v.color || "#FF6B00", is_active: v.is_active ?? 1, sort_order: v.sort_order || 0 });
    });
  }, [id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      if (isEdit) await api.put(`/process-steps/${id}`, form);
      else await api.post("/process-steps", form);
      showToast("success", isEdit ? "Paso actualizado" : "Paso creado");
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title={isEdit ? "Editar Paso" : "Nuevo Paso"} description={isEdit ? "Modifica el paso del proceso." : "Agrega un nuevo paso."}
        breadcrumbs={[{ label: "Pasos", to: "/process-steps" }, { label: isEdit ? "Editar" : "Nuevo" }]} icon={<Settings size={20} />}
        action={<button onClick={() => navigate("/process-steps")} className="mp-btn-ghost text-xs"><ArrowLeft size={14} /> Volver</button>} />
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="mp-card p-6 space-y-4">
            ...
            <div>
              ...
              <input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} className="mp-input text-sm" placeholder="Ej: Recepción" required />
            </div>
            <div>
              <label className="block text-xs font-medium text-[var(--mp-text-secondary)] mb-1.5">Descripción</label>
              <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="mp-input text-sm min-h-[100px]" placeholder="Describe este paso..." />
            </div>
          </div>
        </div>
        <div className="space-y-6">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => navigate("/process-steps")} className="mp-btn-ghost flex-1 text-xs">Cancelar</button>
